...
station_serve_A = distinct(tidy_nyc_transit, line, station_name, .keep_all = TRUE) %>% 
  filter(route_name == "A")
```
The number of distinct station that serve the A train is `r nrow(station_serve_A)`, and among those stations, `r sum(station_serve_A$ada)`
